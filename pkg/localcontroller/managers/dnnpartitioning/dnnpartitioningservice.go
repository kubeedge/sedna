...
// JointInferenceManager defines dnn-partitioning-service manager
type Manager struct {
...
func (jm *Manager) monitorWorker() {
	for {
		...
		if err := jm.Client.WriteMessage(um, header); err != nil {
			klog.Errorf("dnn-partitioning(name=%s) uploads worker(name=%s) message failed, error: %v",
				name, workerMessage.Name, err)
		}
	}
}

// Insert inserts dnn-partitioning-service config in db
