...
class OBSClientWrapper():
    def __init__(
        ...
        LOGGER.info("Authenticating with the bucket and setting up communication backend.")
        self._bucket_auth()
    ...
    def _bucket_auth(self, retry=1):
        try:
            ...
            bucket_auth_endpoint = f"{self.server}/rest/boto3/s3/bucket-auth?vendor={self.vendor}&region={self.region}&bucketid={self.bucket_name}&apptoken={self.app_token}"
    ...
    def _get_s3_client_list(self, retry=1):
        ...
        s3_client_list = []
        try:
            query_urls_endpoint = f"{self.server}/rest/boto3/s3/query/csb-file-server/all/ip-and-port?vendor={self.vendor}&region={self.region}"
            ...
            if result_dict['fileServerUrlList'] is not None:
                for csb_file_server_url in result_dict['fileServerUrlList']:
                    ...
                    csb_obs_service_endpoint = f"{self.server}/rest/boto3/s3/{self.vendor}/{self.region}/{self.app_token}"
    ...
    def download_single_object(self, remote_path, local_path=".", failed_count=1, selected_index=0):
        try:
            wait_select_index_list = [index for index in range(len(self.s3_client_list))]
            ...
            objectKey_base64 = base64.urlsafe_b64encode(remote_path.encode(encoding="utf-8"))
            objectKey_base64 = str(objectKey_base64, encoding="utf-8")
            resp = s3_client.get_object(Bucket=self.bucket_name, Key=objectKey_base64)
            ...
            if failed_count >= 3:
                ...
                self.download_single_object(remote_path, local_path=".", failed_count=failed_count, selected_index=selected_index)

    def list_objects(self, remote_path, next_marker=''):
        LOGGER.info(f"Listing objects in folder {remote_path}.")
        try:
            encoded_path = base64.urlsafe_b64encode(remote_path.encode(encoding="utf-8"))
            encoded_path = str(encoded_path, encoding="utf-8")

            list_objects_endpoint = f"{self.server}/rest/boto3/s3/list/bucket/objectkeys?vendor={self.vendor}&region={self.region}&bucketid={self.bucket_name}&apptoken={self.app_token}&objectkey={encoded_path}&nextmarker={next_marker}"
            ...
            if bool(util.strtobool(result_dict['success'])):
                for file in result_dict["objectKeys"]:
                    LOGGER.info(f"Found {file['objectKey']} of size {file['size']}.")

        except Exception as ex:
            LOGGER.error(f"Unable to list objects {ex}")


    def upload_file(self, local_folder_absolute_path, filename, bucket_path, failed_count=1, selected_index=0):
        ...
        try:
            ...
            with open(os.path.join(local_folder_absolute_path, filename), 'rb') as file:
                _ = s3_client.put_object(Bucket=self.bucket_name, Key=key, Body=file.read())
            ...
            if failed_count < 3:
                failed_count += 1
                time.sleep(failed_count)
                self.upload_file(local_folder_absolute_path, filename, bucket_path, failed_count=failed_count, selected_index=selected_index)
