...
class Producer(Client):
    ...
    def publish_data_synchronous(self, data, topic = "default"):
        ...
        for t in topic:
            try:
                ...
                LOGGER.info(f"Message published on topic {record_metadata.topic} with offset {record_metadata.offset}.")
    ...
    def publish_data_asynchronous(self, data, topic = "default"):
        ...
        for t in topic:
            try:
                self.producer.send(t, data).add_callback(self.on_send_success).add_errback(self.on_send_error)
