...
class JointInference(JobBase):
   ...
   """
    ...
    def initial(self, run_type="cloud", **kwargs):
        if callable(self.estimator):
            self.estimator = self.estimator()
        if not os.path.exists(self.model_path):
            raise FileExistsError(f"{self.model_path} miss")
        else:
            self.estimator.load(self.model_path, model_name=run_time)
        if run_type == "cloud":
            self.is_cloud = True
            app_server = InferenceServer(model=self, servername=self.job_name,
                                         host=self.remote_ip, http_port=self.port)
            app_server.start()
        else:
    ...
    def inference(self, data=None, post_process=None, **kwargs):
        ...
        sednaLogger.info("Joint Inference Experiment Inference Start")
        res = self.estimator.predict(data, **kwargs)
        sednaLogger.info("Joint Inference Experiment Inference Done")
